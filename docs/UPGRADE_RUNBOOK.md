...
> 下次「merge 开源分支并升级」直接照此执行即可。最近一次执行:**0.1.160 → 0.1.163**(2026-07-22,245 提交,`deploy/docker-compose.yml` 1 个文件 2 处冲突已解决)。
...
> 0.1.163 经验:同一 compose 再次冲突(1 个文件 2 处):镜像名和 Redis volume。保留 `sub2api:local` 与 Redis host bind mount,同时吸收上游 `UPDATE_GITHUB_TOKEN` / `REDIS_USERNAME`、PostgreSQL 调优参数,以及 Redis 命令每行末尾 `\\` 的关键修复。
...
>
> 0.1.163 经验:`corepack prepare pnpm@9` 曾因 npm registry 瞬时网络错误失败一次。先用相同 Node 镜像和
> 相同命令最小复现确认不是兼容问题;若复现成功,保持参数不变重试构建即可,BuildKit 会复用已下载层。
>
